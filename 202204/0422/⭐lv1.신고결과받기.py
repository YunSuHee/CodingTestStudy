...
def solution(id_list, report, k):
    ...
    for i in report:
        user, reporteduser = i.split()
        user_report_history.append([user, reporteduser])
    user_report_history = set(list(map(tuple, user_report_history)))

    for i in user_report_history:
        user_report_num[i[1]] += 1

    for i, j in user_report_num.items():

        if j >= k:
            for l in user_report_history:
                if l[1] == i:
                    answer[l[0]] += 1
